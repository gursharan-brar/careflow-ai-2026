...
from db import get_db, generate_id, calculate_wait, get_next_queue_position, now_iso
...
def triage():
    ...
    conn = get_db()
    cur = conn.cursor()
    cur.execute("SELECT id, name, visit_type, status FROM visits WHERE id = ?", (visit_id,))
    visit = cur.fetchone()
